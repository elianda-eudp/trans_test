...
from fabric.api import hosts, env,run,execute,task,cd
import xlrd
import xlsxwriter
import datetime
import sys
...
filename = u'D:\邮储理财项目组工作\新技术\Python\pythonsrc\\tran_test\测试用例.xlsx'
...
def excel_table_byindex(file= filename,colnameindex=0,by_index=0):
    ...
    for rownum in range(1,nrows):
        ...
        #写ini文件文件
        file_name=u'D:\邮储理财项目组工作\新技术\Python\pythonsrc\\tran_test\ini\\' + trade_code + '_' + table.row(rownum)[0].value + '.ini'
...
t = paramiko.Transport(("10.136.2.24",22))
...
localpath=u'D:\邮储理财项目组工作\新技术\Python\pythonsrc\\tran_test\ini'  
'''因为\t为特殊字符，所有需要转义'''
...
for f in files:
    ...
    print results
    tran_out_str= results[u'10.136.2.24'].split('\r\n')
    #print tran_out_str
...
def init(worksheet):
    ...
    tongguolv=float(data1[u'test_success'])/float(data1[u'test_sum'])
...
def test_detail(worksheet):
    ...
    cell_format = workbook.add_format({u'bold': True ,u'align': u'center',u'valign': u'vcenter',u'font_color': u'#ffffff',u'fg_color': u'#DC143C'})
    ...
    #    print item
        ...
        #print chardet.detect(no_match_dict[item])
        #worksheet.set_row(int(item[6:]), None, cell_format)
        rownum=int(item[6:])+1
        rownum_str=str(rownum)
        worksheet.write( u"A" + rownum_str, item, cell_format)
        worksheet.write( u"B" + rownum_str, all_dict[item[6:]], cell_format)
        worksheet.write( u"C" + rownum_str, no_match_dict[item], cell_format)
        worksheet.write( u"D" + rownum_str, u'结果与预期不匹配', cell_format)
        #_write_center(worksheet, u"A" + rownum_str, item, workbook)
        #_write_center(worksheet, u"B" + rownum_str, all_dict[item[6:]], workbook)
        #_write_center(worksheet, u"C" + rownum_str, no_match_dict[item], workbook)
        #_write_center(worksheet, u"D" + rownum_str, u'结果与预期不匹配', workbook)
    ...
    for item in match_dict.iterkeys():
        ...
        _write_center(worksheet, u"A" + rownum_str, item, workbook)
        _write_center(worksheet, u"B" + rownum_str, all_dict[item[6:]], workbook)
        _write_center(worksheet, u"C" + rownum_str, match_dict[item], workbook)
        _write_center(worksheet, u"D" + rownum_str, u'结果与预期匹配', workbook)
